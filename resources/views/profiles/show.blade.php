...
<div class="container-fluid mx-1">
  <div class="row">
      <div class="bg-white col-lg-8 col-md-12 px-0">
          <section class="">
                <div>
                    ...
                    <div class="d-flex w-100 justify-content-between" style="height: 80px">
                        ...
                        @can ('update', $user->profile)
                            <a href="{{ route('profiles.edit',['profile' => $user->profile]) }}"
                                class="btn btn-outline-primary w-25 mt-3 font-weight-bolder rounded-pill me-3"
                                style="font-size: larger;height: 42px;margin-right: 10px;">
                                Edit profile
                            </a>
                        @else
                            <!-- <a href="#"
            ...
            <div class="px-4">
               {{-- @php
                ...
                <div class="d-flex justify-content-around h5">
                    ...
                    <a href="{{ route('followlist', ['user' => $user]) }}" class="text-decoration-none">
                        <strong>{{ ($user->following) ? count($user->following) : 0 }}</strong> Following
                    </a>
                    <a href="{{ route('followlist', ['user' => $user]) }}" class="text-decoration-none">
                        <strong>{{ ($user->followers) ? count($user->followers) : 0 }}</strong> Followers
                    </a>
                </div>
            </div>
